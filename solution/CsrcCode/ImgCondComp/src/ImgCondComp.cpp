...
void Compress(const vector<Mat>& tars, vector<Mat>& cmpx, Alphabet& alphabet,
  vector<FCM_TYPE>& refFcms, vector<Ctx>& refCtxs, vector<FCM_TYPE>& tarFcms,
  vector<Ctx>& tarCtxs, double gamma, vector<double>& nBits) {
	...
	for(unsigned n = 0 ; n < tars.size() ; ++n) {
		...
		for(auto it = tars[n].begin<uchar>() ; it != tars[n].end<uchar>() ; ++it) {
			...
			for(unsigned ctx = 0 ; ctx < tarCtxs.size() ; ++ctx) {
				...
				weights[refCtxs.size() + ctx] /= totalWeight; // Renormalize weights
				for(unsigned s = 0 ; s < alphabet.size() ; ++s)
					weightedProbs[s] += tarFcms[ctx].probs[s] *
			...
			for(unsigned ctx = 0 ; ctx < refCtxs.size() ; ++ctx) {
				...
				weights[ctx] = pow(weights[ctx], gamma) * refFcms[ctx].probs[*it];
				totalWeight += weights[ctx];
			}
			...
			for(unsigned ctx = 0 ; ctx < tarCtxs.size() ; ++ctx) {
				// Update the "accumulated" probability generated by the
				// model along the image
				weights[refCtxs.size() + ctx] = pow(weights[refCtxs.size() + ctx],
				  gamma) * tarFcms[ctx].probs[*it];
...
int main(int argc, char *argv[]) {
	...
	for(int n = 1 ; n < argc ; n++)
		if(string(argv[n]) == "-R") {
			resize = stod(argv[n+1]);
			if(resize > 1 || resize <= 0) {
				cerr << "Error: Resize should belong to ]0, 1]" << endl;
				return 1;
			}
	...
	for(int n = 1 ; n < argc ; n++)
		if(string(argv[n]) == "-r") {
			Mat img = imread(argv[n+1], CV_LOAD_IMAGE_GRAYSCALE);
			if(img.empty()) {
			...
			tempRefs.push_back(img);

			if(verbose)
				printf("Reference %d is %dx%d\n", (int)refs.size(), img.rows, img.cols);
	...
	for(int n = 1 ; n < argc ; n++)
		if(string(argv[n]) == "-t") {
			...
			tempTars.push_back(img);

			if(verbose)
				printf("Target %d is %dx%d\n", (int)tars.size(), img.rows, img.cols);

			tarSize += img.rows * img.cols;

			if(createCmpx)
				cmpx.push_back(Mat(img.size(), CV_64F));

		}

	if(tempRefs.size() + tempTars.size() == 0) {
		cerr << "Error: no image was provided" << endl;
		return 1;
	}
	// Perform image resize
	if(resize != 1.0){
		for(unsigned n = 0 ; n < tempRefs.size() ; ++n){
			
			Mat dst(50, 50, tempRefs[n].depth());

			IplImage tmp = tempRefs[n];
			IplImage iplDst = dst;
		...
		for(unsigned n = 0 ; n < tempTars.size() ; ++n){
			cout << round(tempTars[n].cols*resize) << endl;
			cout << round(tempTars[n].rows*resize) << endl;

			Mat dst(50,50, tempTars[n].depth());
	...
	for(int n = 1 ; n < argc ; ++n) {
		if(string(argv[n]) == "-rc") {
			...
			if(!ifs.is_open()) {
				cerr << "Error: could not open context file \"" <<
				  argv[n+1] << "\"" << endl;
				return 1;
			}
			...
			while(ifs >> order) {
				...
				if(!(ifs >> alphaN && ifs >> tmp && tmp == '/' && ifs >> alphaD)) {
	...
	Compress(tars, cmpx, alphabet, refFcms, refCtxs, tarFcms, tarCtxs, gamma, nBits);
 
	if(createCmpx)
		for(unsigned n = 0 ; n < tars.size() ; ++n) {
			auto mm = minmax_element(cmpx[n].begin<double>(), cmpx[n].end<double>());

			for(auto it = cmpx[n].begin<double>() ; it != cmpx[n].end<double>() ; ++it)
				*it = (uchar)((log(*it) - log(*mm.first)) /
				  (log(*mm.second) - log(*mm.first)) * 255 + 0.5);
